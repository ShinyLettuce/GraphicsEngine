...
InsaneShape::InsaneShape() = default;
InsaneShape::~InsaneShape() = default;

bool InsaneShape::Initialize(ID3D11Device* aDevice, const float aScreenPosition[2], const float aColor[3])
{
	HRESULT result;

	Vertex vertices[5]
	{
		{ aScreenPosition[0] + -0.2f, aScreenPosition[1] + -0.2f, 0, 1, aColor[0],aColor[1], aColor[2], 1},
		{ aScreenPosition[0] + -0.2f, aScreenPosition[1] + 0.2f, 0, 1, aColor[0],aColor[1], aColor[2], 1},
		{ aScreenPosition[0] + 0.2f, aScreenPosition[1] + 0.2f, 0, 1,  aColor[0],aColor[1], aColor[2], 1},
		{ aScreenPosition[0] + 0.2f, aScreenPosition[1] + -0.2f, 0, 1, aColor[0],aColor[1], aColor[2], 1},
		{ aScreenPosition[0], aScreenPosition[1] + -0.45f, 0, 1, aColor[0],aColor[1], aColor[2], 1},
	};
	...
	std::string vsData;
	{
		...
		std::ifstream psFile;
		psFile.open("InsanePixelShader.cso", std::ios::binary);
...
void InsaneShape::Render(ID3D11DeviceContext* aDeviceContext)
{
	...
	aDeviceContext->DrawIndexed(9, 0, 0);
}
